...
def distribution(data, transformed=False, plt_show=True):
    ...
    for i, feature in enumerate(["capital-gain", "capital-loss"]):
        ...
        y_ticks = list(
            np.round(np.quantile(np.arange(0, max_count + 1), [0, 0.25, 0.5, 0.75])).astype(int)
        )
...
def evaluate(results, accuracy, f1, plt_show=True):
    ...
    for k, learner in enumerate(results.keys()):
        for j, metric in enumerate(
            ...
            for i in np.arange(3):
                # Creative plot code
                ax[j // 3, j % 3].bar(
                    i + k * bar_width, results[learner][i][metric], width=bar_width, color=colors[k]
                )
    ...
    if plt_show is True:
        ...
        plt.savefig(IMAGES_FOLDER + "evaluate.png")
...
def feature_plot(importances, X_train, y_train, plt_show=True):
    ...
    plt.bar(
        np.arange(5), values, width=0.6, align="center", color="#00A000", label="Feature Weight"
    )
    plt.bar(
        np.arange(5) - 0.3,
        np.cumsum(values),
    ...
    if plt_show is True:
        ...
        plt.savefig(IMAGES_FOLDER + "feature_plot.png")
...
if __name__ == "__main__":
    ...
    evaluate(test_results_evaluate, test_accuracy_evaluate, test_f1_evaluate, plt_show=False)
    ...
    feature_plot(
        test_importances_feature_plot,
        test_X_train_feature_plot,
        test_y_train_feature_plot,
        plt_show=False,
    )
